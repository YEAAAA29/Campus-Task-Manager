...
export default function AddEditTaskScreen({ navigation, route }) {
  ...
  const [dueDateText, setDueDateText] = useState(
    // Show existing date as a formatted string in the input
  ...
  // ── DATE PARSING ───────────────────────────────────────
  // We use a plain text input for the date (avoiding the DateTimePicker
  // dependency complexity for this tutorial). The user types a date
  // string and we validate/parse it on change.
  //
  // TRADE-OFF: A real app would use @react-native-community/datetimepicker
  // for better UX. The plain-text approach here is simpler to explain
  // and avoids native module linking complexity.
  const handleDateTextChange = useCallback((text) => {
    setDueDateText(text);
    setDateError('');

    if (!text.trim()) {
      // Empty text = "no due date" — valid
      setDueDate(null);
      return;
    }
    ...
    if (isNaN(parsed.getTime())) {
      // Invalid date string
      setDateError('Use format: MM/DD/YYYY or Jun 10, 2026');
      setDueDate(null);
    } else {
      // Valid: set time to end of day (11:59 PM) so the whole day counts
  ...
  // Keeping validation in a function (not inline in handleSave)
  ...
  function validate() {
    ...
    if (dateError) {
      return 'Please fix the due date.';
    }
  ...
  const handleSave = useCallback(() => {
    ...
    navigation.goBack();
  }, [title, description, category, priority, dueDate, dateError, isEditMode]);
