...
### 2. Plain TextInput for Dates
Instead of loading a native DatePicker component (like `@react-native-community/datetimepicker`), the form uses a simple `TextInput` with manual date validation and parsing.
* *Trade-off*: A date picker provides better UX for a production app, but standard text inputs make the JS code fully visible, platform-independent, and easy to run in simulators without dealing with complex native cocoapod linkages.
